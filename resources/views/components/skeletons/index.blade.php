<div id="skeleton" class="w-full p-2 md:p-6">
    <div class="skeleton h-4 w-32 rounded-xl mb-6 md:py-1"></div>

    <div class="mb-6 flex justify-between py-6">
        <div class="h-10 w-40 skeleton rounded-xl mb-6"></div>

        <div class="grid justify-items-start gap-4">
            <div class="skeleton h-14 w-48 rounded-full"></div>
        </div>
    </div>

    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-16 mt-6 p-2 lg:px-6">
        <div class="skeleton h-32"></div>
        <div class="skeleton h-32"></div>
        <div class="skeleton h-32"></div>
        <div class="skeleton h-32"></div>
    </div>

    <div class="col-1 skeleton md:col-span-2 h-125 mt-6"></div>

</div>